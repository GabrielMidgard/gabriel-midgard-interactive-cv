...
interface CastleWallsPositionProps {
  startVw: number;
}

export function CastleWallsBackground({ startVw }: CastleWallsPositionProps) {
  return <div className={styles.scenario} style={{ left: `${startVw}vw` }} aria-hidden="true" />;
}

export function CastleWallsWorld({ startVw }: CastleWallsPositionProps) {
  return (
    <section
      className={`zone ${styles.profileZone}`}
      style={{ left: `${startVw + 100}vw` }}
    ...
    >
        ...
        <div className={`gate ${styles.levelGate}`}><b>NIVEL 3</b></div>
    </section>
  );
}

export function CastleWallsOverlay({ layerTransform, startVw }: CastleWallsOverlayProps) {
  const wallTransform = { transform: layerTransform(startVw + 260) };
  ...
  return (
    <>
      ...
      <div className={styles.foregroundPost} style={{ transform: layerTransform(startVw + 72) }} aria-hidden="true">
